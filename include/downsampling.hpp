...
std::shared_ptr<OutputPointCloud> voxelgrid_sampling(const InputPointCloud& points, double leaf_size) {
  ...
  for (size_t i = 0; i < traits::size(points); i++) {
    const Eigen::Array3i coord = fast_floor(traits::point(points, i) * inv_leaf_size) + coord_offset;
  ...
  size_t num_points = 0;
  Eigen::Vector3d sum_pt = traits::point(points, coord_pt.front().second);
  for (size_t i = 1; i < traits::size(points); i++) {
    if (coord_pt[i - 1].first != coord_pt[i].first) {
//        traits::set_point(*downsampled, num_points++, sum_pt / sum_pt.z());
      traits::set_point(*downsampled, num_points++, sum_pt/sum_pt.z());
      sum_pt.setZero();
    }
