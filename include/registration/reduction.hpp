...
struct SerialReduction {
  ...
  std::tuple<Eigen::Matrix<double, 3, 3>, Eigen::Matrix<double, 3, 1>, double> linearize(
    ...
    Eigen::Matrix<double, 3, 3> sum_H = Eigen::Matrix<double, 3, 3>::Zero();
    Eigen::Matrix<double, 3, 1> sum_b = Eigen::Matrix<double, 3, 1>::Zero();
    double sum_e = 0.0;
  ...
  double error(const TargetPointCloud& target, const SourcePointCloud& source, const Eigen::Isometry2d& T, std::vector<Factor>& factors) const {
    double sum_e = 0.0;
    for (size_t i = 0; i < factors.size(); i++) {
