...
struct GaussNewtonOptimizer {
  ...
  RegistrationResult optimize(
    ...
    for (int i = 0; i < max_iterations && !result.converged; i++) {
      ...
      // Solve linear system
      const Eigen::Matrix<double, 3, 1> delta = (H + lambda * Eigen ::Matrix<double, 3, 3>::Identity()).ldlt().solve(-b);
      ...
      result.T_target_source = result.T_target_source * se2_exp(delta);
      result.iterations = i;
      result.H = H;
      result.b = b;
      result.error = e;
  ...
  RegistrationResult optimize(
    ...
    for (int i = 0; i < max_iterations && !result.converged; i++) {
      ...
      for (int j = 0; j < max_inner_iterations; j++) {
        ...
        const Eigen::Isometry2d new_T = result.T_target_source * se2_exp(delta);
//        std::cout<<"!!!!!!!!!!!!translation:"<<(result.T_target_source * se2_exp(delta)).translation()<<std::endl;
        double new_e = reduction.error(target, source, new_T, factors);
        general_factor.update_error(target, source, new_T, &e);

        if (verbose) {
          std::cout << "iter=" << i << " inner=" << j << " e=" << e << " new_e=" << new_e << " lambda=" << lambda << " dt=" << delta.tail<2>().norm()
...
//          std::cout<<"..............translation:"<<se2_exp(delta).translation()<<"..............rotation:"<<delta[2]<<std::endl;
