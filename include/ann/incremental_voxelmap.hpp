...
public:
  ...
  void insert(const PointCloud& points, const Eigen::Isometry2d& T = Eigen::Isometry2d::Identity()) {
    // Insert points to the voxelmap
    for (size_t i = 0; i < traits::size(points); i++) {
      const Eigen::Vector3d pt = T * traits::point(points, i);
  ...
  size_t nearest_neighbor_search(const Eigen::Vector3d& pt, size_t* index, double* sq_dist) const {
  ...
  size_t knn_search(const Eigen::Vector3d& pt, size_t k, size_t* k_indices, double* k_sq_dists) const {
    const Eigen::Vector3i center = fast_floor(pt * inv_leaf_size).template head<3>();
...
struct Traits<IncrementalVoxelMap<VoxelContents>> {
  ...
  static Eigen::Vector3d point(const IncrementalVoxelMap<VoxelContents>& voxelmap, size_t i) {
  ...
  static Eigen::Vector3d normal(const IncrementalVoxelMap<VoxelContents>& voxelmap, size_t i) {
    const size_t voxel_id = voxelmap.voxel_id(i);
    const size_t point_id = voxelmap.point_id(i);
    const auto& voxel = voxelmap.flat_voxels[voxel_id]->second;
    return traits::normal(voxel, point_id);
  }
  static Eigen::Matrix3d cov(const IncrementalVoxelMap<VoxelContents>& voxelmap, size_t i) {
    const size_t voxel_id = voxelmap.voxel_id(i);
    const size_t point_id = voxelmap.point_id(i);
    const auto& voxel = voxelmap.flat_voxels[voxel_id]->second;
  ...
  static size_t nearest_neighbor_search(const IncrementalVoxelMap<VoxelContents>& voxelmap, const Eigen::Vector3d& pt, size_t* k_index, double* k_sq_dist) {
    return voxelmap.nearest_neighbor_search(pt, k_index, k_sq_dist);
  }

  static size_t knn_search(const IncrementalVoxelMap<VoxelContents>& voxelmap, const Eigen::Vector3d& pt, int k, size_t* k_index, double* k_sq_dist) {
...
template <typename VoxelContents>
std::vector<Eigen::Vector3d> voxel_points(const IncrementalVoxelMap<VoxelContents>& voxelmap) {
...
template <typename VoxelContents>
std::vector<Eigen::Vector3d> voxel_normals(const IncrementalVoxelMap<VoxelContents>& voxelmap) {
...
template <typename VoxelContents>
std::vector<Eigen::Matrix3d> voxel_covs(const IncrementalVoxelMap<VoxelContents>& voxelmap) {
